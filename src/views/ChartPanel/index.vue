          ...
          <el-form id="formPanel" size="mini" class="analysis-form">
            <el-form-item id="dimensionInput" :label="$t('chart.dimensions')">
              <draggable v-model="dimensions" :group="{name: 'col',pull: true, put: true}" class="draggable-wrapper" @change="handleDimensionChange">
                <el-tag v-for="col in dimensions" :key="col.id" class="draggable-item" size="small" closable @close="handleCloseDimensionTag(col)">
                  {{ col.title }}
                </el-tag>
              </draggable>
            </el-form-item>
            ...
            <el-form-item id="fieldInput" :label="$t('chart.values')">
              <draggable v-model="caculCols" :group="{name: 'col',pull: true, put: true}" class="draggable-wrapper" @change="handleColChange">
                <el-tag v-for="col in caculCols" :key="col.id" size="small" closable class="selected-field" @close="handleCloseColTag(col)">
                  <!-- <el-select v-model="col.calculFunc" class="draggable-item" size="mini" closable style="background: rgba(0,0,0,0);">
                    <el-option v-for="(item, optIndex) in col.availableFunc" :key="optIndex" :label="`${col.title}(${item.name})`" :value="item.func" />
                  </el-select> -->
                   {{ col.title }}
                </el-tag>
              </draggable>
...
import { getTableRows } from '@/api/source'
...
export default {
  ...
  computed: {
    ...
    queryParams() {
      ...
      //   limit: this.limit
      // })
      const allFields = [].concat(this.dimensions, this.caculCols)
      const fields = allFields.map(dim => dim.title).join(',')
      return {
        fields,
        tableId: this.dataSrc.tableId,
        tableName: this.dataSrc.tableName,
        sourceId: this.dataSrc.sourceId,
        sourceName: this.dataSrc.sourceName,
        selectedCalcul: this.caculCols,
        orderByStrs: this.orderByStrs
      }
    }
  ...
  watch: {
    queryParams(value) {
      if (value && value.sourceName && value.tableName) {
  ...
  methods: {
    async fetchData(params) {
      const { list } = await getTableRows(params)
      this.result = list
      // if (this.loading) {
